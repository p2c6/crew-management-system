...
import { Crew, EditCrewForm, StoreCrewForm } from '@/types/Crew'
...
const form = useForm<EditCrewForm>({
  rank_id: props.crew.rank.id,
  first_name: props.crew.first_name,
  middle_name: props.crew.middle_name,
  last_name: props.crew.last_name,
  address: props.crew.address,
  birth_date: props.crew.birth_date 
    ? parseDate(props.crew.birth_date) 
    : null,
  email: props.crew.email,
  weight: props.crew.weight,
  height: props.crew.height,
});
...
const updateCrew = () => {
    ...
    onSuccess: () => {
      form.reset()
      handleFormSuccess()
      toast.success('Crew has been updated')
    },
    onError: (error) => {
...
<template>
  <form @submit.prevent="updateCrew">
    <div class="grid grid-rows-5 gap-5 mb-2">
